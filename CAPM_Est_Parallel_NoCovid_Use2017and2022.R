  ...
  for (empresa in colnames(final_df_filtrado)) {
    if (grepl("_Return$", empresa)) {
      ...
      tryCatch({
        ...
        variables_adicionales <- final_df_filtrado[, c("Date", "Rm_Rf", "Rm_Rf2", "Rm_Rf3", "Exc_Rate", "Brent", "Soy", "Passangers", "VIX")]
        ...
        fit_result <- stable_lm_custom(
          formula = Retorno ~ Rm_Rf + Rm_Rf2 + Rm_Rf3 + Exc_Rate + Brent + Soy + Passangers + VIX,
          data = emp_df,
          bounds = bounds
        )
...
procesar_empresa <- function(empresa) {
  tryCatch({
    ...
    market_returns <- final_df_filtrado[, c("Date", "Rm_Rf", "Rm_Rf2", "Rm_Rf3", "Exc_Rate", "Brent", "Soy", "Passangers", "VIX")]
...
exportar_a_excel_openxlsx <- function(resultados_empresas, parametros_iniciales, archivo_salida) {
  ...
  for (i in seq_along(resultados_empresas)) {
    ...
    fila_empresa <- data.frame(
      ...
      Rm_Rf2 = ifelse(length(resultado$coefs) >= 7, resultado$coefs[7], NA),
      Rm_Rf3 = ifelse(length(resultado$coefs) >= 8, resultado$coefs[8], NA),
      ExcRate = ifelse("Exc_Rate" %in% names(resultado$coefs), resultado$coefs["Exc_Rate"], NA),
      Brent = ifelse("Brent" %in% names(resultado$coefs), resultado$coefs["Brent"], NA),
      Soy = ifelse("Soy" %in% names(resultado$coefs), resultado$coefs["Soy"], NA),
      Passangers = ifelse("Passangers" %in% names(resultado$coefs), resultado$coefs["Passangers"], NA),
      VIX = ifelse("VIX" %in% names(resultado$coefs), resultado$coefs["VIX"], NA),
      AIC = resultado$aic,
      BIC = resultado$bic,
      MSE = resultado$mse,
...
archivo_salida <- paste0("C:/Users/alexi/OneDrive - Universidad Técnica Federico Santa María/2025/Paper CAPM/Análisis/Salidas_Estable/results_est_cv_", start_year, "_to_", end_year, ".xlsx")
#archivo_salida <- paste0("C:/Users/usuario/OneDrive - Universidad Técnica Federico Santa María/2025/Paper CAPM/Análisis/Salidas_Estable/results_est_cv_", start_year, "_to_", end_year, ".xlsx")
...
#carpeta_histogramas <- "C:/Users/usuario/OneDrive - Universidad Técnica Federico Santa María/2025/Paper CAPM/Gráficos/Estable/carpeta_histogramas_est_2224"
carpeta_histogramas <- "C:/Users/alexi/OneDrive - Universidad Técnica Federico Santa María/2025/Paper CAPM/Gráficos/Estable/carpeta_histogramas_est_cv_1724"
